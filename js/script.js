const leadForm = document.querySelector('#order_form');
const userChat = document.querySelector('.chat-content-list');
const loadCommentButton = document.querySelector('.glow-button');
let illness = [];
...
loadCommentButton.addEventListener('click', function() {
  ...
  document.querySelectorAll('.comment-item').forEach(comment => {
    if(!comment.classList.value.includes('comment-item-show')) {
...
function genderNext() {
  ...
  $(".chooseGenderM").click(() => {
    ...
    leadForm.insertAdjacentHTML('beforeend', '<input type="hidden" name="gender" value="male">');
  ...
  $(".chooseGenderW").click(() => {
    ...
    leadForm.insertAdjacentHTML('beforeend', '<input type="hidden" name="gender" value="female">');
...
function appAge() {
  ...
  let data = setInterval(() => {
    ...
    if (empty_field != "" && full_month != "" && year != "") {
      ...
      leadForm.insertAdjacentHTML('beforeend', `<input type="hidden" name="birth_day" value="${selectS}">`);
...
function YsNo() {
  ...
  $("#yeas").click(() => {
    ...
    leadForm.insertAdjacentHTML('beforeend', '<input type="hidden" name="is_allergy" value="positive">');
  ...
  $("#no").click(() => {
    ...
    leadForm.insertAdjacentHTML('beforeend', '<input type="hidden" name="is_allergy" value="negative">');
...
function Photofoot() {
  ...
  userChat.insertAdjacentHTML('beforeend', 
  `<div class="chat-content-buttons-gender ill-container">
    <div class="chat-content-buttons-gender-block ill">
      <span class="chooseGenderM illness" data-value="Головокружение">Головокружение</span>
    </div>
    <div class="chat-content-buttons-gender-block ill">
      <span class="chooseGenderW illness" data-value="Усталость">Усталость</span>
    </div>
    <div class="chat-content-buttons-gender-block ill">
      <span class="chooseGenderW illness" data-value="Боль в затылке">Боль в затылке</span>
    </div>
    <div class="chat-content-buttons-gender-block ill">
      <span class="chooseGenderW illness" data-value="Бессоница">Бессоница</span>
    </div>
    <div class="chat-content-buttons-gender-block ill">
      <span class="chooseGenderW illness" data-value="Усиленное сердцебиение">Усиленное сердцебиение</span>
    </div>
    <div class="chat-content-buttons-gender-block ill">
      <span class="chooseGenderW illness" data-value="Носовые кровотечения">Носовые кровотечения</span>
    </div>
    <div class="chat-content-buttons-gender-block ill">
      <span class="chooseGenderW illness" data-value="Звон в ушах">"Звон" в ушах</span>
    </div>
    <div class="chat-content-buttons-gender-block ill">
      <span class="chooseGenderW illness" data-value="Нервозность">Нервозность</span>
    </div>
    <div class="chat-content-buttons-gender-block sendButton">
      <button>Отправить</button>
    </div>
  </div>`);

  document.addEventListener('click', function({ target }) {
    if(target.classList.value.includes('illness')) {
      if(illness.includes(target.textContent)) {
        illness = illness.filter(el => el !== target.textContent);
        target.parentElement.classList.remove('active-illnes');
      } else {
        illness.push(target.textContent);
        target.parentElement.classList.add('active-illnes');
      }
    }

    if(target.classList.value.includes('sendButton') || target.parentElement.classList.value.includes('sendButton')) {
      leadForm.insertAdjacentHTML('beforeend', `<input type="hidden" name="illness" value="${illness.join(',')}">`);
      addArray();
      process = true;
    }
  });
...
function validateForm(event) {
  ...
  const isValidLength = this.phone.value.replace(regex, '').length == 12 || this.phone.value.replace(regex, '').length == 13;
...
leadForm.phone.addEventListener('input', function(event) {
  const regex = new RegExp(/\D/gi);

  if(regex.test(event.data) || this.value.replace(regex, '').length > 13) {
    if(!(event.data == null)) {
      this.value = '+38' + this.value.slice(3, -1);
    }
  }
...
document.querySelector('.scroll-me').addEventListener('click', scrollDown);
